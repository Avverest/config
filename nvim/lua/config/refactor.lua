-- Рефакторинг по всему проекту.
--
-- Три разных инструмента под три разные задачи:
--   1. переименование символа   — по смыслу, через LSP (понимает области
--      видимости, не трогает одноимённые переменные в других функциях);
--   2. замена текста            — тупой поиск по всему проекту, для строк,
--      комментариев, конфигов и языков без LSP;
--   3. переименование файлов    — с обновлением импортов силами LSP-сервера.

local map = vim.keymap.set

-- ---------------------------------------------------------------------------
-- 1. Переименование символа везде, где он упоминается.
--
-- Под капотом всё тот же vim.lsp.buf.rename(), но inc-rename показывает
-- изменения прямо по мере набора нового имени — во всех открытых буферах
-- сразу, до подтверждения. Отменяется <Esc>, применяется <CR>.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. Поиск и замена по всему проекту (grug-far).
--
-- Открывается обычным буфером: сверху поля «что искать / чем заменить /
-- фильтры по файлам», снизу живой список совпадений. Работает на ripgrep,
-- поэтому видит весь проект, а не только открытые файлы.
-- Внутри буфера: <localleader>r — выполнить замену, <localleader>s —
-- синхронизировать правки обратно в файлы, g? — подсказка по клавишам.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. Переименование и перемещение файлов (mini.files).
--
-- Файловое дерево редактируется как обычный текст: правишь имена строк,
-- удаляешь, вставляешь новые — потом нажимаешь `=`, и mini.files применяет
-- накопленные изменения. Перед применением он рассылает LSP-серверам
-- workspace/willRenameFiles, чтобы те поправили ссылки на файл.
--
