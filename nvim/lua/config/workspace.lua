-- Рабочая область: окна (<leader>w) и вкладки (<leader>t).
-- Прыжков по цифрам намеренно нет: переключение последовательное или пикером.

local map = vim.keymap.set

-- splitright/splitbelow выставлены в options.lua: окно появляется справа/снизу
...
-- Zoom: своей команды в Vim нет. Открываем окно в новой вкладке (`tab split`)
-- вместо сохранения раскладки: «вернуть как было» — это закрыть вкладку.
-- Флаг t:zoomed отличает такую вкладку от обычной.
...
-- Имена вкладок.
--
-- Хранятся в переменной вкладки t:tabname. Строка вкладок сверху отключена
-- (showtabline=0, см. config/buffers.lua), поэтому имя видно в пикере вкладок
-- (<leader>tt) — этого достаточно, чтобы понять, какая вкладка под какую задачу.
-- ---------------------------------------------------------------------------
